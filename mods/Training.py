...
class TheArenaGame(bs.CoopGameActivity):
    ...
    def __init__(self,settings={}):
        ...
        except Exception: self._preset = 'default'
        
        self._excludePowerups = ['iceBombs','tripleBombs','stickyBombs','curse','punch']

        # for each bot type: spawn rate, increase, dIncrease
        self._botSpawnTypes = {bs.BomberBotProShielded:    [1.0,  0.0,  0.0],
                               bs.ToughGuyBotProShielded:  [1.0,  0.0,  0.0],
                               bs.ChickBotProShielded:     [1.0,  0.0,  0.0],
                               bs.NinjaBotProShielded:     [1.0,  0.0,  0.0],
                               bs.BunnyBot:                [0.3,  0.05, 0.0],
                               bs.MelBotStatic:            [0.1,  0.03, 0.001],
                               bs.PirateBotNoTimeLimit:    [0.05, 0.02, 0.002]
                               }
    ...
    def _startBotUpdates(self):
        self._botUpdateInterval = min(1500 - 500*(len(self.players)), 500)
        self._updateBots()
        self._updateBots()
    ...
    def _dropPowerup(self,index,powerupType=None):
        if powerupType is None: powerupType = bs.Powerup.getFactory().getRandomPowerupType(excludeTypes=self._excludePowerups)
    ...
    def _dropPowerups(self,standardPoints=False,forceFirst=None):
        ...
        if standardPoints:
            ...
            for i,pt in enumerate(pts):
                bs.gameTimer(1000+i*500,bs.WeakCall(self._dropPowerup,i,forceFirst if i == 0 else None))
        else:
    ...
    def doEnd(self,outcome):
        ...
        self.end(delay=2000,results={'outcome':outcome,'score':self._score,'playerInfo':self.initialPlayerInfo})
        
    def _updateBots(self):
        self._botUpdateInterval = max(500,self._botUpdateInterval * 0.98)
        self._botUpdateTimer = bs.Timer(int(self._botUpdateInterval),bs.WeakCall(self._updateBots))

        botSpawnPoints = [[-5,10,-4.14],[0,10,-4.14],[5,10,-4.14]]
        dists = [0,0,0]
        playerPts = []
        ...
        for i in range(3):
            for p in playerPts: dists[i] += abs(p[0]-botSpawnPoints[i][0])
        ...
        if dists[0] > dists[1] and dists[0] > dists[2]:
            ...
            pt = botSpawnPoints[1]
        else: pt = botSpawnPoints[2]

        pt = (pt[0]+3.0*(random.random()-0.5),pt[1],2.0*(random.random()-0.5)+pt[2])
        ...
        # after every spawn we adjust our ratios slightly to get more difficult..
        for t in self._botSpawnTypes.items():
    ...
    def handleMessage(self,m):
        ...
        if isinstance(m,bs.PlayerSpazDeathMessage):
            ...
            if m.killerPlayer is not None and m.killerPlayer.exists():
                ...
                try:
                    self.scoreSet.playerScored(m.killerPlayer,pts,target=target,kill=True,screenMessage=False,importance=importance)
    ...
    def __superHandleMessage(self,m):
        super(TheArenaGame,self).handleMessage(m)
