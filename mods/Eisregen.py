...
def bsGetLevels():
    return [bs.Level('Eisregen',displayName='${GAME}',gameType=EisregenGame,settings={},previewTexName='rampagePreview'),
            bs.Level('EisregenEpic',displayName='${GAME}',gameType=EisregenGame,settings={'Epic Mode':True},previewTexName='rampagePreview')]



class EisregenGame(bs.TeamGameActivity):
    ...
    def _dropBomb(self,position,velocity):
        ...
        if rand33 == 1:
            ...
            bType1 = "sticky"
            bRad = 4.0
        b = bs.Bomb(position=position,velocity=velocity,bombType=bType1,blastRadius=bRad).autoRetain()
        if rand33 == 3 or rand33 == 4:
            b.arm()

    def _decrementMeteorTime(self):
        self._meteorTime = max(10,int(self._meteorTime*0.93))
