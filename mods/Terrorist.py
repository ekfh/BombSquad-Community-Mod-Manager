...
class BrainFreezeGame(bs.TeamGameActivity):
    ...
    def getDescription(cls,sessionType):
        return 'Survive the attack as long as possible'
    ...
    def onBegin(self):
        ...
        t = 1000 if len(self.players) > 2 else 1000
        if self.settings['Epic Mode']: t /= 4
        bs.gameTimer(t,self._decrementMeteorTime,repeat=False)
        ...
        bs.gameTimer(5000,bs.Call(self._bots.spawnBot,bs.ToughGuyBotProShielded,pos=(-4,3,-2),spawnTime=1000))
        bs.gameTimer(10000,bs.Call(self._bots.spawnBot,bs.ToughGuyBotProShielded,pos=(5,3,-2),spawnTime=1000))
        bs.gameTimer(15000,bs.Call(self._bots.spawnBot,bs.ToughGuyBotProShielded,pos=(-5,3,-2),spawnTime=1000))
        bs.gameTimer(20000,bs.Call(self._bots.spawnBot,bs.ToughGuyBotProShielded,pos=(6,3,-2),spawnTime=1000))
        bs.gameTimer(25000,bs.Call(self._bots.spawnBot,bs.ToughGuyBotProShielded,pos=(-6,3,-2),spawnTime=1000))
        bs.gameTimer(30000,bs.Call(self._bots.spawnBot,bs.ToughGuyBotProShielded,pos=(7,3,-2),spawnTime=1000))
        bs.gameTimer(35000,bs.Call(self._bots.spawnBot,bs.ToughGuyBotProShielded,pos=(-7,3,-2),spawnTime=1000))
    ...
    def _dropBombCluster(self):
        ...
        if False:
            bs.newNode('locator',attrs={'position':(12,6,-5.5)})
            bs.newNode('locator',attrs={'position':(9,6,-2.3)})
    ...
    def _dropBomb(self,position,velocity):
        b = bs.Bomb(position=position,velocity=velocity,bombType='normal').autoRetain()
